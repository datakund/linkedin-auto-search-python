...
A python library that uses browser automation to search on linkedin automatically
...
```sh
pip install linkedin-search-python
```

### Import

```javascript
from linkedin-search-python import *
```

### Login

To search on linkedin first we will login to linkedin. There are two ways of login:-
* Credentials
* Cookies
...
To search on linkedin we use ``search`` method.
It requires **keyword** as input parameter

```javascript
linkedin.search(keyword="place_search_term_here")
```
